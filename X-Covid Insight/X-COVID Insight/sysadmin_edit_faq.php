...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (isset($_POST['faq_submit'])) { 
            $updateResult = $db2->faq->updateOne(
            ...
            if ($updateResult->getMatchedCount() === 0) {
                throw new Exception("❌ No matching FAQ found.");
            ...
            $deleteResult = $db2->faq->deleteOne(['_id' => $objectId]);

            if ($deleteResult->getDeletedCount() === 0) {
                throw new Exception("❌ No FAQ deleted.");
            }
            ...
            $insertResult = $db2->faq->insertOne([
                'question' => $question,
                'answer' => $answer
            ]);
...
<head>
    ...
    <title>Manage FAQs - X-COVID Insight</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <aside class="w-64 bg-white p-6 rounded-xl shadow-xl h-fit sticky top-6">
            ...
            <nav>
                <ul class="space-y-2">
                    ...
                    <li>
                        <a href="sysadmin_edit_faq.php" class="flex items-center gap-3 p-3 rounded-lg bg-blue-50 text-blue-600 font-medium">
        ...
        <div class="flex-1 space-y-8">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-800">FAQ Manager</h1>
            ...
            <div class="bg-white rounded-2xl shadow-sm">
                <div class="p-6 border-b border-gray-100">
                    <div class="flex items-center justify-between">
                        ...
                        <button id="addFaqBtn" class="flex items-center gap-2 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg transition-all">
                    ...
                    <form method="POST" action="" class="bg-gray-50 rounded-xl p-5 shadow-sm animate-card hover:shadow-md transition-shadow">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="answer" class="w-full px-3 py-2 border rounded-lg h-32 resize-none focus:ring-2 focus:ring-purple-500"><?= htmlspecialchars($faq['answer']) ?></textarea>
                            </div>
                            
                            <div class="flex justify-end space-x-2">
                                <button type="submit" name="faq_submit" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                                    Save Changes
                                </button>
    ...
    <!-- Add FAQ Modal -->
    <div id="addFaqModal" class="fixed inset-0 z-50 flex items-center justify-center modal-bg hidden">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-8 relative">
            <button id="closeAddFaqModal" class="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl">&times;</button>
            <h3 class="text-xl font-bold mb-4 text-gray-800">Add New FAQ</h3>
            <form id="addFaqForm" method="POST" action="" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Question</label>
                    <input type="text" name="question" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-purple-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Answer</label>
                    <textarea name="answer" required class="w-full px-3 py-2 border rounded-lg h-32 resize-none focus:ring-2 focus:ring-purple-500"></textarea>
                </div>
                <div class="flex justify-end">
                    <button type="submit" name="add_faq" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                        Add FAQ
                    </button>
                </div>
            </form>
    ...
    closeAddFaqModal.addEventListener('click', () => {
        addFaqModal.classList.add('hidden');
    });
    
    addFaqModal.addEventListener('click', (e) => {
        if (e.target === addFaqModal) {
            addFaqModal.classList.add('hidden');
        }
    });

    // Existing form logic for edit/delete
    document.querySelectorAll('form').forEach(form => {
        if (form.id === 'addFaqForm') return; // skip add form
    ...
    addFaqForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        const submitBtn = addFaqForm.querySelector('button[type="submit"]');
        const originalText = submitBtn.innerHTML;
