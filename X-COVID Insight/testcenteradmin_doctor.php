...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['doctor_id'])) {
    try {
        $doctor = $db->doctor->findOne(['_id' => new MongoDB\BSON\ObjectId($_POST['doctor_id'])]);
        $newStatus = $doctor['status'] === 'active' ? 'suspended' : 'active';
        
        $result = $db->doctor->updateOne(
            ['_id' => new MongoDB\BSON\ObjectId($_POST['doctor_id'])],
            ['$set' => ['status' => $newStatus]]
        );
        
        if ($result->getModifiedCount() > 0) {
            $_SESSION['success'] = "Doctor status updated to $newStatus!";
        } else {
...
try {
    ...
    $doctors = $db->doctor->find($filter)->toArray();
} catch (Exception $e) {
    ...
    <div id="doctorModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-2xl p-6">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-bold">Doctor Details</h3>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p id="modal-experience" class="mt-1"></p>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="font-medium">Certification:</label>
                        <a id="modal-certification" target="_blank" class="text-blue-600 hover:underline mt-1 block"></a>
                    </div>
    ...
    <div class="min-h-screen flex">
        ...
        <aside class="bg-white w-25 hover:w-72 fixed h-full shadow-lg transition-all duration-300 ease-in-out overflow-y-auto overflow-x-hidden z-40">
            <div class="p-4 flex flex-col h-full">
                ...
                <nav class="space-y-4 flex-1">
                    ...
                    <a href="testcenteradmin_doctor.php" class="flex items-center p-3 bg-blue-100 text-blue-700 hover:bg-blue-50 rounded-xl group">
        ...
        <main class="flex-1 ml-20 transition-all duration-300 p-8">
            ...
            <div class="flex justify-between items-center mb-8">
                ...
                <a href="testcenteradmin_createdoctor.php" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">
            ...
            <!-- Search Form -->
            <div class="mb-6">
                <form method="GET" action="testcenteradmin_doctor.php" class="flex gap-4">
                    <input type="text" name="search" placeholder="Search by name, email, or specialization" 
                    ...
                    <?php if (isset($_GET['search']) && $_GET['search'] !== ''): ?>
                        <a href="testcenteradmin_doctor.php" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition">
                            Clear
                        </a>
                    <?php endif; ?>
                </form>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php if (!empty($doctors)): ?>
                            ...
                            <tr class="table-row">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"><?php echo $doctor['specialization'] ?? 'N/A'; ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900"><?php echo $doctor['phone'] ?? 'N/A'; ?></div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button onclick="viewDoctor('<?= $doctor['_id'] ?>')" 
                                    class="text-blue-600 hover:text-blue-900 mr-4">
                                        <i class="fas fa-eye mr-1"></i>View
                                    </button>
                                    <!-- Add Edit Button -->
                                    <a href="edit_doctor.php?id=<?= $doctor['_id'] ?>" 
                                    class="text-green-600 hover:text-green-800 mr-4">
                                        <i class="fas fa-edit mr-1"></i>Edit
                                    </a>
                                    <form method="POST" class="inline">
                                        <input type="hidden" name="doctor_id" value="<?= $doctor['_id'] ?>">
                                        <button type="submit" class="<?= $doctor['status'] === 'active' ? 'text-yellow-600 hover:text-yellow-800' : 'text-green-600 hover:text-green-800' ?>">
                                            <i class="fas fa-power-off mr-1"></i>
                                            <?= $doctor['status'] === 'active' ? 'Suspend' : 'Unsuspend' ?>
                                        </button>
                                    </form>
                                </td>
                            </tr>
    ...
    <script>
        async function viewDoctor(doctorId) {
            try {
                ...
                document.getElementById('modal-location').textContent = doctor.location;
                document.getElementById('modal-experience').textContent = `${doctor.experience} years`;
                document.getElementById('modal-status').textContent = doctor.status;
                document.getElementById('modal-certification').href = doctor.certification;
                document.getElementById('modal-certification').textContent = 'View Certification';
